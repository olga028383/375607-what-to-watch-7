...
describe('Component: RatingField', () => {
  it('there must be a correct renderer', () => {
    ...
    render(<RatingField index={KEY} value={5} handleRatingChange={handleRatingChange} key={KEY}/>);
  ...
  it('checked should change', () => {
    const handleRatingChange = jest.fn();
    const {rerender} = render(<RatingField index={KEY} value={5} handleRatingChange={handleRatingChange} key={KEY}/>);
